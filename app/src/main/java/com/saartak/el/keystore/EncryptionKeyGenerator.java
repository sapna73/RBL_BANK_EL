package com.saartak.el.keystore;
...
public class EncryptionKeyGenerator {
  private static final String TAG=EncryptionKeyGenerator.class.getCanonicalName();
  ...
  @TargetApi(Build.VERSION_CODES.M) static SecurityKey generateSecretKey(KeyStore keyStore) {
    try {
      ...
      Log.d(TAG,"generateSecretKey ==> "+e.getMessage());
    }
    try {
  ...
  static SecurityKey generateKeyPairPreM(Context context, KeyStore keyStore) {
    try {
      ...
      Log.d(TAG,"generateKeyPairPreM ==> "+e.getMessage());
    }
  ...
  static SecurityKey generateSecretKeyPre18(Context context) {
    ...
    try {
      ...
      Log.d(TAG,"generateSecretKeyPre18 ==> "+"Secret Key Fetched :" + new String(pkEntry.getSecretKey().getEncoded(), "UTF-8"));
      ...
      Log.d(TAG,"generateSecretKeyPre18 ==> "+e.getMessage());
    }
    return null;
  }
  ...
  private static boolean loadKeyStore(Context context, KeyStore androidCAStore, char[] password) {
    ...
    try {
      ...
      Log.d(TAG,"loadKeyStore ==> "+e.getMessage());
      return false;
    }
    try {
  ...
  private static void saveMyKeystore(Context context, KeyStore androidCAStore, char[] password,
    ...
    Log.d(TAG,"saveMyKeystore ==> "+"Secret Key Saved : " + new String(mySecretKey.getEncoded(), "UTF-8"));
  }
}
