...
import com.saartak.el.adapter.LeadToApplicationAdapter;
...
import static com.saartak.el.constants.AppConstant.SCREEN_NAME_UPSTREAM_DOWNSTREAM;
import static com.saartak.el.constants.AppConstant.STAGE_ID_ZERO;
...
public class LeadToApplicationActivity extends LOSBaseActivity implements LeadToApplicationAdapter.SyncCallbackInterface {
    ...
    TextView tvClientName,clientId;
                                        ...
                                        ERROR_MESSAGE_SYNC_SUCCESS, new ConfirmationDialog.ActionCallback() {
                                            ...
                                            public void onAction() {
                                                ...
                                                if (viewModel.getMyStageLiveData() != null) {
                                                    ...
                                                    viewModel.getMyStageLiveData().observe(LeadToApplicationActivity.this, getMyStagesObserver);
                                                }
                                            }
                                        });
                            } else {
...
//                    break;
    ...
    public void submitCallback(int position, MasterTable masterTable) {
       ...
       // getIndividualAndOnIndividual(masterTable);
    ...
    /* private void getVKYCDownStreamServiceData(String userId, String clientId, String loanType, String moduleType,MasterTable masterTable) {
         try {
             appHelper.getDialogHelper().getLoadingDialog().showGIFLoading();
             String uniqueId = String.valueOf(System.currentTimeMillis());
             viewModel.getVKYCDownStreamServiceData(userId, CLIENT_ID, loanType, MODULE_TYPE_GENERATE_CIBIL);
             if (viewModel.getDownStreamResponseDTOLiveData() != null) {
                 Observer observer = new Observer() {
                     @Override
                     public void onChanged(Object o) {
                         appHelper.getDialogHelper().getLoadingDialog().closeDialog();
                         DownStreamResponseDTO downStreamResponseDTO = (DownStreamResponseDTO) o;
                         viewModel.getDownStreamResponseDTOLiveData().removeObserver(this);
                         if (downStreamResponseDTO.getErrorMessage()!=null&&downStreamResponseDTO.getErrorMessage().equalsIgnoreCase("")) {
                             syncAllScreenByClient(masterTable);
                         }else {
                             if(downStreamResponseDTO.getErrorMessage()!=null){
                                 appHelper.getDialogHelper().getConfirmationDialog().show(ConfirmationDialog.ALERT, downStreamResponseDTO.getErrorMessage());
                             }
                         }

                     }
                 };
                 viewModel.getDownStreamResponseDTOLiveData().observe(this, observer);
             }
         } catch (Exception ex) {
             ex.printStackTrace();
             appHelper.getDialogHelper().getLoadingDialog().closeDialog();
 ...
 */
    private void getUpstreamDownStreamData(String clientId, MasterTable masterTable) {
        try {
            viewModel.getRawDataByClientIDAndModuleType(SCREEN_NAME_UPSTREAM_DOWNSTREAM, clientId, MODULE_TYPE_GENERATE_CIBIL);
            if (viewModel.getRawTableLiveData() != null) {
                Observer getLeadRawDataObserver = new Observer() {
                    @Override
                    public void onChanged(@Nullable Object o) {
                        ...
                        if (rawDataTableList != null && rawDataTableList.size() > 0) {
                            for (RawDataTable rawDataTable : rawDataTableList) {
                                ...
                                CibilUpStreamDownStreamModel cibilUpStreamDownStreamModel = new Gson().fromJson(rawData, CibilUpStreamDownStreamModel.class);
                                isUpStream = cibilUpStreamDownStreamModel.getIsUpStreamSucces();
                                cibilUpStreamDownStreamModelList = cibilUpStreamDownStreamModel;
                                try {
                                    updateMasterTableStatus(masterTable, FINAL_STATUS_SUBMITTED);
                                } catch (Exception ex) {
                                    ex.printStackTrace();
                            ...
                            getCIBILTableListFromDB(masterTable);
                        }
                    }
                };
    ...
    void getIndividualAndOnIndividual(MasterTable masterTable) {

        viewModel.getRawDataByClientIDAndModuleType(SCREEN_NAME_LEAD, masterTable.getClientId(), MODULE_TYPE_LEAD);
        if (viewModel.getRawTableLiveData() != null) {
            Observer getLeadRawDataObserver = new Observer() {
                @Override
                public void onChanged(@Nullable Object o) {
                            ...
                            if (rawDataTable != null) {
                                ...
                                if (hashMap != null && hashMap.size() > 0) {
                                    if (hashMap.containsKey(TAG_NAME_INDIVIDUAL_OR_NONINDIVIDUAL)) {
                                        if (hashMap.get(TAG_NAME_INDIVIDUAL_OR_NONINDIVIDUAL).toString().equalsIgnoreCase(TAG_NAME_NONINDIVIDUAL)) {
                                            try {
                                                //updateMasterTableStatus(masterTable, FINAL_STATUS_SUBMITTED);
                                            } catch (Exception ex) {
                                                ex.printStackTrace();
                                            ...
                                            getUpstreamDownStreamData(masterTable.getClientId(), masterTable);
                                        }
                                    }
                                }
                            }
    ...
    private void getCIBILTableListFromDB(MasterTable masterTable) {
        try {
            viewModel.getCIBILTableListFromDB(masterTable.getClientId(), masterTable.getLoan_type());
    ...
    public void getSyncWorkFlowData(String clientId, String isSync, String workFlowId) {
        try {
            appHelper.getDialogHelper().getLoadingDialog().showGIFLoading();
            viewModel.getSyncWorkFlowData(clientId, isSync, workFlowId);
            if (viewModel.getGetSyncWorkflowResponseTableLiveData() != null) {
                Observer observer = new Observer() {
                    @Override
                    public void onChanged(Object o) {
                        appHelper.getDialogHelper().getLoadingDialog().closeDialog();
                        ArrayList<SyncWorkflowResponseTable> data = (ArrayList<SyncWorkflowResponseTable>) o;
                        viewModel.getGetSyncWorkflowResponseTableLiveData().removeObserver(this);
                        if (data != null && data.size() > 0) {

                            try {
                                if (Integer.valueOf(data.get(0).getStatus()) == 1) {
                ...
                viewModel.getGetSyncWorkflowResponseTableLiveData().observe(this, observer);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            appHelper.getDialogHelper().getLoadingDialog().closeDialog();
