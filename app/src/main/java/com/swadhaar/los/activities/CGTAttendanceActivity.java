...
import com.swadhaar.los.adapter.AttendanceAdapter;
...
import static com.swadhaar.los.dynamicui.constants.ParametersConstant.ERROR_MESSAGE_CAPTURE_ATTENDANCE_REASON;
import static com.swadhaar.los.dynamicui.constants.ParametersConstant.ERROR_MESSAGE_CGT_DETAILS_EMPTY;

public class CGTAttendanceActivity extends LOSBaseActivity {

    private static final String TAG = CGTAttendanceActivity.class.getCanonicalName();
...
//    TextView tvStaffId, tvStaffName, tvSOBDate;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(cgtJsonString)) {
            ...
            if (CGT_TABLE != null) {
                if( ! TextUtils.isEmpty(CGT_TABLE.getCenterName())) {
                    tvCenterName.setText(CGT_TABLE.getCenterName());
                }
                if( ! TextUtils.isEmpty(CGT_TABLE.getCenterId())) {
                    tvCenterId.setText(CGT_TABLE.getCenterId());
                }
                if( ! TextUtils.isEmpty(CGT_TABLE.getVillageName())) {
                    tvVillageName.setText(CGT_TABLE.getVillageName());
                }

                if(CGT_TABLE.isCycleOneCompleted()){
                    tvCgtCycle.setText(CONSTANT_CGT_CYCLE_2); // TODO: CGT CYCLE 2
                }else {
                    tvCgtCycle.setText(CONSTANT_CGT_CYCLE_1); // TODO: CGT CYCLE 1
                }
            }
        }
...
//        attendanceAdapter = new AttendanceAdapter(AttendanceActivity.this, new ArrayList<>(),
//                appHelper,"");
//        rvLeadDetails.setAdapter(attendanceAdapter);
        ...
        searchByPhoneNo.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                ...
                if (attendanceAdapter != null && attendanceAdapter.getFilter() != null) {
        ...
        btnSaveAttendance.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(attendanceAdapter !=null && attendanceAdapter.getCgtAttendanceTableList() !=null
                        && attendanceAdapter.getCgtAttendanceTableList().size()>0){
                    if(CGT_TABLE !=null) {
                        boolean isAttendanceReasonCaptured=false;
                        for(CGTAttendanceTable cgtAttendanceTable : attendanceAdapter.getCgtAttendanceTableList()){
                            if(CGT_TABLE.isCycleOneCompleted()){
                                // TODO: Check CGT 2 Attendance
                                if(cgtAttendanceTable.isCGT2Attendance()){
                                    isAttendanceReasonCaptured=true;
                                }
                                else if( ! cgtAttendanceTable.isCGT2Attendance() && ! TextUtils.isEmpty(cgtAttendanceTable.getCGT2AbsentReason())){
                                    isAttendanceReasonCaptured=true;
                                }else{
                                ...
                                // TODO: Check CGT 1 Attendance
                                if(cgtAttendanceTable.isCGT1Attendance()){
                                    isAttendanceReasonCaptured=true;
                                }
                                else if( ! cgtAttendanceTable.isCGT1Attendance() && ! TextUtils.isEmpty(cgtAttendanceTable.getCGT1AbsentReason())){
                                    isAttendanceReasonCaptured=true;
                                }else{
                                    isAttendanceReasonCaptured=false;
                                    break;
                        ...
                        if(isAttendanceReasonCaptured) {
                            saveAttendanceDetails(attendanceAdapter.getCgtAttendanceTableList());
                        }else{
                        ...
                        appHelper.getDialogHelper().getConfirmationDialog().show(ConfirmationDialog.ALERT,
                                ERROR_MESSAGE_CGT_DETAILS_EMPTY);
                    }
                }
            }
        });
    ...
    private void saveAttendanceDetails(List<CGTAttendanceTable> cgtAttendanceTableList) {
        try{
            viewModel.updateCGTTableAttendance(cgtAttendanceTableList,CGT_TABLE);
            if (viewModel.getCgtAttendanceTableLiveDataList() != null) {
                Observer observer = new Observer() {
                    @Override
                    public void onChanged(@Nullable Object o) {
                        viewModel.getCgtAttendanceTableLiveDataList().removeObserver(this);
                ...
                viewModel.getCgtAttendanceTableLiveDataList().observe(this, observer);
            }
        }catch (Exception ex){
            ex.printStackTrace();
        }
    ...
    public void configureViewModel() {
        ...
        if (CGT_TABLE != null) {
            if(CGT_TABLE.isCycleOneCompleted()){
                getCGTAttendanceTable(CGT_2);
            }else {
                getCGTAttendanceTable(CGT_1);
            }
        }
    }
...
//                                attendanceAdapter.setItem(cgtTableList);
//                            } else {
//                                attendanceAdapter = new AttendanceAdapter(AttendanceActivity.this, cgtTableList,appHelper,CGT_TABLE);
//                                rvLeadDetails.setAdapter(attendanceAdapter);
...
//    }
    ...
    public void getCGTAttendanceTable(String cgtCycle) {
        appHelper.getDialogHelper().getLoadingDialog().showGIFLoading();

        try {
            viewModel.getCGTAttendanceTable(cgtCycle,CLIENT_ID); // TODO: Here client id is nothing but center id
            if (viewModel.getCgtAttendanceTableLiveDataList() != null) {
                Observer observer = new Observer() {
                    @Override
                    public void onChanged(@Nullable Object o) {
                        appHelper.getDialogHelper().getLoadingDialog().closeDialog();
                        List<CGTAttendanceTable> cgtAttendanceTableList = (List<CGTAttendanceTable>) o;
                        viewModel.getCgtAttendanceTableLiveDataList().removeObserver(this);
                        if (cgtAttendanceTableList != null && cgtAttendanceTableList.size() > 0) {
                            ...
                            if (attendanceAdapter != null) {
                                attendanceAdapter.setItem(cgtAttendanceTableList,cgtCycle);
                            } else {
                                attendanceAdapter = new AttendanceAdapter(CGTAttendanceActivity.this,
                                        cgtAttendanceTableList, appHelper, cgtCycle);
